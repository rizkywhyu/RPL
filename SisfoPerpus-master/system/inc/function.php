...
function list_kategori() {
	...
	$query = query("SELECT idkategori, nama_kategori FROM tb_kategori ");
	while ($row = mysql_fetch_row($query)) {
	
			echo "<li><a href='index.php?mod=system/page&pg=produk&idkategori=".$row[0]."'>" . ucwords($row[1]) . "</a> </li>";
...
function list_news($jumlah) {
	...
	$query = query("SELECT idberita,judul FROM tb_berita order by tanggal desc limit $jumlah");
	while ($row = mysql_fetch_row($query)) {
	
			echo "<li><a href='index.php?mod=system/page&pg=berita&idberita=".$row[0]."'>" . ucwords($row[1]) . "</a> </li>";
...
function list_merek() {
	...
	$query = query("SELECT idmerek, nama_merek FROM tb_merek");
	while ($row = mysql_fetch_row($query)) {
...
function update_status_login($status,$idpelanggan) {
	...
	query("update tb_pelanggan set status='$status' where idpelanggan='$idpelanggan'");
}
function count_stat(){
	if(get_today_stat()<1){
...
function combo_kategori($kode) {
	...
	$query = query("SELECT *   FROM tb_kategori");
	while ($row = mysql_fetch_row($query)) {
		if ($kode == "")
...
function combo_produk($kode) {
	...
	$query = query("SELECT idproduk,nama_produk   FROM tb_produk");
	while ($row = mysql_fetch_row($query)) {
		if ($kode == "")
			echo "<option value='$row[0]'> " . ucwords($row[1]) . " </option>";
		else
...
function konversi_tanggal($time) { /*konversi_tanggal*/
	list($thn, $bln, $tgl) = explode('-', $time);
	$tmp = $tgl . " " . konversi_bulan($bln) . " " . $thn;
	return $tmp;
}

function tampil_tanggal($time) { /*konversi_tanggal*/
	list($thn, $bln, $tgl) = explode('-', $time);
	$tmp = $tgl . " " . konversi_bulan($bln) . " " . $thn;
	return $tmp;
}
